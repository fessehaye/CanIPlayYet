...
exports.handler = async function(event, context) {
  ...
  return axios
    .get(
      `https://api.challonge.com/v1/tournaments/${slug}/matches.json?api_key=${API_KEY}`
    )
    .then(response => {
      if (response.error) {
        return {
      ...
      if (total === 0) {
        throw { error: 'No Event Data Avaialble' };
      }
      ...
      const inProgress = matches.filter(
        m => m.match.underway_at !== null && m.match.state === 'open'
      ).length;
